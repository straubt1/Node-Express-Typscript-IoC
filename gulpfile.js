...
gulp.task('clean', 'Cleans the generated js files from lib directory', function () {
  return del([
    'lib/**/*',
    '!lib/config.json',//keep local config file around please :)
  ]);
});
...
gulp.task('bundle', 'Concat the generated JS files into a single app', ['build'], function () {
  gulp.src('lib/*.js')
    .pipe(concat('app.js'))
    .pipe(gulp.dest('lib'));
});

gulp.task('run', 'Runs the node app on the designated port', ['build'], function (cb) {
  exec('node lib\\rest.js', function (err, stdout, stderr) {
    console.log(stdout);
    console.log(stderr);
    cb(err);
  });
